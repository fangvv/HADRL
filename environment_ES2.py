...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self):
        ...
        #            backhaul_rate):
        self.Server = [Server((i + 1), 'server%d' % (i + 1), np.zeros(2), 1, 60 * (10 ** 9),
        ...
        self.Tasks = [Task((i + 1), 2000, 400, 700, 0.8) for i in range(self.num_task)]
    ...
    def _get_obs_samll(self):
        ...
        # distance1, distance2 = self.distance()

        # for i in distance1:
        #     state_small.append(i / 100)
        # for i in distance2:
        #     state_small.append(i / 100)
    ...
    # 这里只是一个简单的适用于2个智能体的
    def trans_action_for_ave(self,action, users_num, server_num ):
        # print("small_action ", action)
        # action 是-1到1 之间的值
        o = action[0:users_num]
        o_new = np.zeros(users_num)
    ...
    def ger_reward_time_energy(self,user, env):
        ...
        if user.get_frequency == 0:
            com_fre = server.get_com_cap * (user.get_frequency + 0.2)
        else:
        ...
        if user.get_bandwidth == 0:
            bandwidth = server.get_bandwidth_cap * (user.get_bandwidth + 0.2)
        else:       
