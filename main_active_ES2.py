...
from environment_ES2 import MultiAgentEnv
...
def train(arglist):
    ...
    for episode_gone in range(arglist.max_episode):
        ...
        if episode_gone % 10 == 0:
            ...
            with open("simulation_energy/reward_large_user30_cpu30_active1.txt", "w") as f:
                for r in rewards_server_cache:
                    f.write(str(r) + '\n')

            with open("simulation_energy/reward_small_user30_cpu30_active1.txt", "w") as f:
                for r_small in rewards_samll:
                    f.write(str(r_small) + '\n')

            with open("simulation_energy/users_time_user30_cpu30_active1.txt", "w") as f:
                for time in users_time:
                    f.write(str(time) + '\n')


            with open("simulation_energy/users_server_user30_cpu30_active1.txt", "w") as f:
                for server in server_switch_cost:
                    f.write(str(server) + '\n')


            with open("simulation_energy/users_hit_user30_cpu30_active1.txt", "w") as f:
...
if __name__ == '__main__':
    ...
    # torch.manual_seed(0)
    # ### cuda
    # torch.cuda.manual_seed(0) # 经过测试,使用cuda时,只加这句就能保证两次结果一致
